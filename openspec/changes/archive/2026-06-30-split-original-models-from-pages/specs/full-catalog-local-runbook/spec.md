...
#### Scenario: County NHGIS cache shape

- **WHEN** preflight validates `nhgis_cache_2018_county_b19013_b01003.json`
- **THEN** the `data` object contains 58-element arrays for `COUNTYA`, `county_population_2018`, and `county_income_2018`

### Requirement: Symmetric CLI documentation

Documentation SHALL list symmetric pipeline entry points:

```bash
python scripts/run_original_models.py
python scripts/export_pages_catalog.py --release-id 2018-2024 --staging-dir <staging-path>
```

Supported environment variables and flags for partial or faster dev builds: `ZILLOW_INPUT_DIR`, `PAGES_SKIP_HIERARCHICAL=1`, and `--max-pairs N`.

#### Scenario: Full build manifest profile

- **WHEN** a non-fixture build completes successfully
- **THEN** `manifest.json` has `input_profile` equal to `release-2018-2024-v1` (not `fixture-v1`)
- **THEN** `catalog.json` contains more than the four fixture pairs
...
### Requirement: Playwright e2e after full build

After a verified full release is promoted to `docs/data/releases/2018-2024/`, the developer SHALL run `scripts/run_explorer_e2e.sh` or `npx playwright test` from `e2e/`. Playwright SHALL NOT run against fixture-only releases.
...
- **THEN** Playwright passes against `input_profile: release-2018-2024-v1`

#### Scenario: Fixture insufficient for Playwright

- **WHEN** only `setup_local_site_test.sh` (fixture) has been run
- **THEN** `run_explorer_e2e.sh` exits non-zero explaining full Cartesian build is required
...
- Models tab renders a chart with populated predictor and outcome dropdowns covering multiple ENT pairs
- Outcome dropdown excludes `_CO_total` and `_BP_total` suffixes
